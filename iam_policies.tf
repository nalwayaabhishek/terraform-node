data "template_file" "s3_full_access"{
    template = "${file("${path.module}/templates/s3_full_access.tpl")}"
    vars{
        bucket_name = "${aws_s3_bucket.S3_BUCKET.bucket}"
    }
}
data "template_file" "codedeploy_full_access"{
    template = "${file("${path.module}/templates/codedeploy_full_access.tpl")}"
}

data "template_file" "ec2_trust_policy"{
    template = "${file("${path.module}/templates/ec2_trust_policy.tpl")}"
}
